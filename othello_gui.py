...
##attempt to fix subprocess error

#########################

class OthelloGui(object):
    #Takes input from terminal, python othello_gui.py Player1 Player2
    ...
    def mouse_pressed(self,event):
        ...
        try:
            ...
            if not get_possible_moves(self.game.board, self.game.current_player):
                if not get_possible_moves(self.game.board, self.game.current_player):
                    self.log("'1' possible moves {}".format(get_possible_moves(self.game.board,self.game.current_player)))
                    if not get_possible_moves(self.game.board, other_player):
                        self.log("'2' possible moves {}".format(get_possible_moves(self.game.board,other_player)))
                        self.log("No Valid Moves for either")
                        self.shutdown("Game Over")
                    else:
                        self.game.current_player = 1 if self.game.current_player == 2 else 2
                        player = "Dark" if self.game.current_player == 1 else "Light"
                        self.log("XXX No Valid Moves for {}".format(player))
                        self.draw_board()
    ...
    #Controls AI moves so important
    def ai_move(self):
        ...
        try:
            ...
            if not get_possible_moves(self.game.board, self.game.current_player):
                if not get_possible_moves(self.game.board, self.game.current_player):
                    self.log("current {}".format(self.game.current_player))
                    self.log("'current' possible moves {}".format(get_possible_moves(self.game.board,self.game.current_player)))
                    if not get_possible_moves(self.game.board, other_player):
                        self.log("other {}".format(other_player))
                        self.log("'other' possible moves {}".format(get_possible_moves(self.game.board,other_player)))
                        self.log("No Valid Moves for either")
                        self.shutdown("Game Over")
                    else:
                        self.game.current_player = 1 if self.game.current_player == 2 else 2
                        player = "Light" if self.game.current_player == 1 else "Dark"
                        self.log("No Valid Moves for {} (skipped turn)".format(player))
...
def main():
    ...
    if len(sys.argv) == 3:
        ...
        p1 = Player(1)
        ##cant seem to find the right file? Subprocess issue
        filename = "nathan_ai.py"
        #p2 = AiPlayerInterface(filename,2)
        p2 = Player(2)
    game = OthelloGameManager(dimension=8)
